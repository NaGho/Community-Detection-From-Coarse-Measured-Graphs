            ...
            for L in L_array:
                ...
                for alpha in alpha_array:
                    for C in C_array:
                        ...
                        if(C*L>n):
                            continue
                        print('log(n)/n=', np.log(n)/n)
                        print('C*sqrt(log(n)/n)=', C*np.sqrt(np.log(n)/n))
                        alpha_beta_Gap = (alpha+beta)/2-np.sqrt(alpha*beta)
        #                 print('alpha-beta Gap={},K/2={}'.format(alpha_beta_Gap, K/2))
                        df_in = {'n':n, 'L':L, 'K':K, 'C':C, 'alpha':alpha, 'beta':beta, 'alpha-beta Gap': alpha_beta_Gap, 'nu':nu}
                        UB_ML_Failure_LogError, df_in = UB_ML_Failure_Error_Function(df_in)
