...
K_array = [5] 
nu_array = [2]
m_array = [10, 20, 50, 100, 150, 200, 300, 400, 500] 

df = pd.read_csv('out_wrt_measurementSize_for_p.csv') 
...
df = df.rename(columns={'Failure UB Error': 'This Paper: Theoretical Error Bound',\
                   ...
                   'm': 'Measurement Size (# of proto-nodes)'})



if(Community_Recover and ErrorBound_Calc):
    x_col = 'Measurement Size (# of proto-nodes)'
    ...
    figName = title + '_wrt_{}_n{}_m{}_K{}_nu{}'.format('MeasurementSize', n_array[0], m_array[0], K_array[0], nu_array[0])
    relPlot(df, x_col=x_col, y_col=y_col, hue_col=var_name, style_col=var_name, plot_core=PlotCore(title='', figName=figName,\
                                   saveFlag=True, showFlag=windows, log_scale=False, aspect=0.6,\
                                        minX_axis=np.min(df[x_col])-2, maxX_axis=np.max(df[x_col])+5,\
                                        minY_axis=np.min(df[y_col])-0.01, maxY_axis=np.max(df[y_col])+0.01, palette_cmap='hot',\
                                                paper_rc = {'lines.linewidth': 4, 'lines.markersize': 8} )) # np.max(df[x_col])+5
    # palettes = ['BuPu', 'hot', 'icefire', 'afmhot', 'Accent' , sns.color_palette("mako_r", 5) , "tab10", 'RdYlBu' , 'Paired' , 'Dark2', "Reds", sns.cubehelix_palette(5)]
