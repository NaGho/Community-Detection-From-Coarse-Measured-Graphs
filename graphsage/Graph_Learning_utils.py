...
from GraphLearning.utils import NN_classifier, RandomForest_classifier, upper_triangle, class_relabel, sec2win, win2sec, inpython_online_wrapper, half1D_to_2D, network_stats_calc
...
class online_miniBatchIterator():
    def __init__(self, data_load_core, placeholders, task_core, load_Core=None, graphL_minibatch=None, batch_size=None, clip_sizes=None):
        self.batch_num = data_load_core.settings_TrainNumFiles 
        self.batch_size = batch_size
        self.placeholders = placeholders
        self.graphL_minibatch = graphL_minibatch
        self.data_load_core = data_load_core 
    ...
    def next(self):
        ...
        self.dataX, self.dataY, sel_win_nums, conv_sizes, clip_sizes = \
                    inpython_online_wrapper(self.matlab_engin, self.data_load_core, [self.batch_num], 'total', self.task_core.data_dir, self.load_Core)
#         X, Y, conv_sizes, sel_win_nums, clip_sizes = self.matlab_engin.python_online_wrapper(self.data_load_core.target, [self.batch_num], 'total', nargout=5)
...
class Hybrid_Rep_Feat():
    ...
    def __init__(self, graphL_core, classif_core, data_load_core, task_core, weight_losses):
        self.task_core = task_core
...
#             A_initial = tf.matrix_set_diag(A_initial, 1.0 * tf.ones((self.graphL_core.num_nodes,))) #[np.diag_indices(self.graphL_core.num_nodes)] = 1
            A_initial = tf.convert_to_tensor((2*self.data_load_core.structural_inf.adj_means[0]-1), dtype=tf.float32)
...
#         print('Final A: \n', (outs_after[0]+1)/2)
    ...
    def train(self, X, Y, target, printing_flag=False, plotting_flag=False):
        print('training ..')
        start_time = common.time.get_seconds()
...
#         self.evel_threshold = self.classif_core.eval_thresh_func(np.vstack(Eval_thresh), axis=0)
        print('    time elapsed: ', common.time.get_seconds()-start_time)  
    ...
    def test(self, X, Y, target, train_test='testing', out_th=None, printing_flag=False, plotting_flag=False, \
        ...
        start_time = time.time() # common.time.get_seconds()
        if(X is not None):
            if(not self.task_core.supervised):
                ...
                outs = self.sess.run([self.graphL_W], feed_dict=feed_dict)
                graphL_Ws = outs[0]
                print('  NCDD graphL: time elapsed to compute %d graphs with %d nodes is %f seconds: ' %( graphL_Ws.shape[0], X.shape[1], time.time()-start_time)) #common.time.get_seconds()
                prob_hat = self.classif_core.disjointed_classifier.predict_proba(graphL_Ws)[:,1]
            ...
            classif_minibatch = online_miniBatchIterator(self.data_load_core, self.placeholders, self.task_core)
        y_hat = None
        prob_hat = None
        W_hat = None
        y_true = None
    ...
    #             y_hat = inn_y if y_hat is None else np.concatenate((y_hat, inn_y))
                ...
                if(plotting_flag): #
                    ...
                    plot_samples(inn_y_true, inn_prob, inn_W, self.data_load_core, self.task_core, clip_sizes, \
                                    sel_win_nums, '_'+str(total_steps) + task_core.feature_mode, num_plots=10*15, mode_plotting='one_clip')
                    
                print(train_test + ' %f percent is done, %d samples so far' % (total_steps*100/self.data_load_core.settings_TestNumFiles, szr_size+non_szr_size) )
                if(not self.task_core.supervised):
...
#             eval_performance(Y, prob_hat, self.classif_core.sensitivity_score_target, th=th)  
        ...
        if(X is not None):
            if(plotting_flag):
                plot_samples(y_true, prob_hat, W_hat, self.data_load_core, self.task_core, clip_sizes, \
...
#         print('final eval-thresholds =%s for sensitivities = %s' %(self.evel_threshold, self.classif_core.sensitivity_score_target))
        for i in range(len(self.data_load_core.structural_inf.adj_means)):
            print('------Adj mode: ' + adj_calc_modes[i])
            print('    Side A mean:  ')
            print(self.data_load_core.structural_inf.adj_means[i])
            print('    A difference ratio: ', np.sum(np.abs(self.final_A_evaluated-self.data_load_core.structural_inf.adj_means[i]))
                                                                /np.size(self.final_A_evaluated))
#             print('    A common ratio: ', np.sum(np.abs(self.final_A_evaluated-data_load_core.structural_inf.adj_means[i]))
#                                                                 /np.size(self.final_A_evaluated))
            print('    Side A var: ')
            print(self.data_load_core.structural_inf.adj_vars[i])
...
def pure_plot_sample(inn_true_y, inn_prob_hat, inn_sel_win_num, counter, num_plots, data_load_core, feats, task_core, W_method):
    ictal_list = np.squeeze(np.argwhere(inn_true_y!=0))
    # determining windows to plot
    try:
        n_pre_szr = int(data_load_core.pre_ictal_num_win)
    except:
        n_pre_szr = int(data_load_core.n_pre_szr)
    if(ictal_list.size>0):
        inter_ictal_list = np.arange(0, np.min((n_pre_szr*2,ictal_list[0]-n_pre_szr*2-data_load_core.window_size_sec*60*10)), 1)
    ...
    inn_feats = [half1D_to_2D(feats[int(i),...], data_load_core.num_nodes) for i in change_arg]
    if(ictal_list.size>0):
    ...
    try:
        plotting_weights('EU_plots/'+str(task_core.target)+'/', str(counter) + W_method, inn_feats, true_states=inn_true_y, \
                     estimated_states=inn_prob_hat, soz_ch_ids=data_load_core.soz_ch_ids, sel_win_num=inn_sel_win_num)
    except:
        print('sdfsdf')
    
    
    
def plot_samples( true_y, prob_hat, feats, data_load_core, task_core, clip_sizes=None, sel_win_nums=None, W_method=' ', num_plots=8*8, mode_plotting='one_clip'):
    print('plotting ..')
    if(mode_plotting == 'aggregate_clips'):
        num_nonszr_plot = 10
        non_szr_plot_prob = np.min((1, num_nonszr_plot/np.shape(data_load_core.y_test)[0]))
        ...
        for counter in np.arange(clip_sizes.shape[1]):
            ...
            pure_plot_sample(inn_true_y, inn_prob_hat, inn_sel_win_num, counter, num_plots, data_load_core, inn_feats, task_core, W_method)
            
    elif(mode_plotting == 'one_clip'):
        pure_plot_sample(true_y, prob_hat, sel_win_nums, 0, num_plots, data_load_core, feats, task_core, W_method)
...
def coarse_eval_performance(Y, prob_hat, sensitivity_score_target, YREF=None, mvAvg_winlen=1, th=None, printing_flag=False, data_load_core=None, counter=0):
    save_folder = 'EU_plots/'+str(data_load_core.target)+'/'
    ...
    if (YREF is None):
        ...
        if(np.any(Y!=0)):
            ...
            szr_onset = preszr_onset + data_load_core.pre_ictal_num_win
            YREF[preszr_onset:szr_onset] = 0
#            Y[szr_offset:np.min((szr_offset + sec2win(3*60, data_load_core) , YREF.size))] = 1 #
            Y[np.max((szr_onset - sec2win(10, data_load_core), 0)):szr_onset] = 1
#             else:
            ...
            if(np.any(Y!=0)):
                ...
                if(np.any(YHAT!=0)):
                    ...
                    try:
                        ...
                        in_delay = win2sec(szr_alarm-szr_onset, data_load_core)
                    except:
